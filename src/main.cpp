...
const unsigned int height_watertank_100percent = 120; // vom Boden, 120cm (in Essencia) ist der Wassertank mit 100% voll betitelt. da der Sensor jedoch 22cm Deadzone hat, ist das mal hier so früh auf 100% definiert. Man müsste den Sensor höher montieren (wie chatgpt/gemini schon gesagt hatte)

// HIER GEÄNDERT: Berechnung angepasst, um den realen Sensorabstand zu reflektieren
...
void loop() {
  ...
  do {
    ...
    if (duration == 0) {
      ...
      u8g2.setCursor(0, 55);
      toggle_var = false; // true wäre für Mess- und Testzwecke. Kann irgendwann ganz entfernt werden, wenn nicht benötigt.
